...
const visible = computed({
  get: () => employeeStore.importResultVisible,
  set: (val) => {
    if (!val) employeeStore.closeImportResult()
  }
})

const result = computed(() => employeeStore.importResult)

const handleClose = () => {
  employeeStore.closeImportResult()
}

const handleDownloadErrors = () => {
  employeeStore.downloadErrorFile()
}
</script>
...
<template>
  <el-dialog
    ...
    align-center
    @close="handleClose"
    class="employee-import-result-dialog"
  >
    <template #header>
...
<style lang="scss">
/* ---- Dialog 容器（unscoped） ---- */
.employee-import-result-dialog.el-dialog {
  width: 480px;
  background: #FFFFFF;
...
.employee-import-result-dialog.el-dialog .el-dialog__header {
  padding: 24px 24px 13px !important;
  margin-right: 0;
  border-bottom: 1px solid #E1E2E9;
}

.employee-import-result-dialog.el-dialog .el-dialog__body {
...
.employee-import-result-dialog.el-dialog .el-dialog__footer {
  padding: 0;
