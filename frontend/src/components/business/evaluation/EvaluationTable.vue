...
import { EVALUATION_TYPE_MAP, EVALUATION_SOURCE_MAP, PROCESS_STATUS_MAP, DISPATCH_TYPE_MAP, PRIORITY_MAP } from '@/constants/evaluation'
...
const emit = defineEmits<{
  ...
  autoDispatch: [evaluation: Evaluation]
  manualDispatch: [evaluation: Evaluation]
}>()
...
/** 自动派单 */
const handleAutoDispatch = (row: Evaluation) => emit('autoDispatch', row)

/** 人工派单 */
const handleManualDispatch = (row: Evaluation) => emit('manualDispatch', row)

/** 判断是否可以派单 */
const canDispatch = (row: Evaluation): boolean => {
...
<template>
  <div ref="tableContainerRef" class="table-container">
    ...
    >
      ...
      <el-table-column prop="title" label="标题/菜品" min-width="150">
        <template #default="{ row }">
          <strong>{{ row.dishName || row.title }}</strong>
        </template>
      </el-table-column>

      <el-table-column prop="reviewerName" label="投诉人" min-width="80" />

      <el-table-column prop="orgName" label="门店" min-width="120" />

      <el-table-column prop="score" label="评分" min-width="120">
        <template #default="{ row }">
          <el-rate v-model="row.score" disabled :max="5" size="small" />
        </template>
      </el-table-column>

      <el-table-column prop="processStatus" label="处理状态" min-width="90">
        <template #default="{ row }">
