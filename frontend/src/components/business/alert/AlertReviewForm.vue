...
import type { AlertReviewDTO, AttachmentItem, AlertReviewResult } from '@/types/alert'
...
const emit = defineEmits<{
  ...
  submit: [data: AlertReviewDTO, done: () => void]
}>()
...
const reviewResult = ref<AlertReviewResult>('approved')
const reviewRemark = ref('')
const reviewAttachments = ref<AttachmentItem[]>([])
...
const isRejected = computed(() => reviewResult.value === 'rejected')
const canSubmit = computed(() => !isRejected.value || reviewRemark.value.trim().length > 0)

const handleClickUpload = () => {
  if (uploading.value) return
  if (reviewAttachments.value.length >= 5) {
...
const handleUpload = async (file: File) => {
  ...
  try {
    ...
    if (res.code === 'SUCCESS' && res.data) {
      reviewAttachments.value.push({ url: res.data.fileUrl, name: res.data.fileName })
    }
  } catch (e) {
...
const handleSubmit = () => {
  ...
  emit('submit', {
    ...
    reviewAttachments: reviewAttachments.value.length > 0 ? reviewAttachments.value : undefined,
  }, () => {
...
<template>
  <el-dialog
    ...
    append-to-body
    destroy-on-close
    class="alert-review-form-dialog"
  >
    <template #header>
      <div class="dialog-header">
        <span class="dialog-title">复核工单</span>
    ...
    <el-form label-width="90px">
      ...
      <el-form-item :label="isRejected ? '驳回原因' : '复核备注'" :required="isRejected">
        <el-input
          v-model="reviewRemark"
          type="textarea"
          :rows="4"
          :placeholder="isRejected ? '请输入驳回原因' : '请输入复核备注（选填）'"
          maxlength="300"
          show-word-limit
        />
      </el-form-item>

      <el-form-item label="复核附件" class="upload-form-item">
        <div>
        ...
        <div v-if="reviewAttachments.length" class="attachment-list">
          <div v-for="(item, idx) in reviewAttachments" :key="idx" class="attachment-item">
...
<style lang="scss">
/* ---- Dialog 容器（unscoped） ---- */
.alert-review-form-dialog.el-dialog {
  background: #FFFFFF;
...
.alert-review-form-dialog .el-dialog__header {
  padding: 24px 24px 13px !important;
  margin-right: 0;
  border-bottom: 1px solid #E1E2E9;
}

.alert-review-form-dialog .el-dialog__body {
...
.alert-review-form-dialog .el-dialog__footer {
  padding: 0;
